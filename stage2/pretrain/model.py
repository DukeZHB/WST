...
class Unet(nn.Module):
    def __init__(
            ...
            out_dim=None,
            dim_mults=(1, 2, 4, 8),
            channels=4,  # 修改：3（原图）+1（掩码）=4通道
            with_time_emb=True,
            with_cat_emb=True,  # 新增：类别嵌入开关
        ...
        if self.encoder_only is False:
            ...
            out_dim = default(out_dim, 3)  # 输出仍为3通道（原图维度）
            self.final_conv = nn.Sequential(
                block_klass(dim, dim), nn.Conv2d(dim, out_dim, 1)
            )

    def forward(self, x, time, cat_label=None):
        # x: (B, 4, H, W) = 3通道原图 + 1通道掩码
        x = self.init_conv(x)
...
class DiffusionNet(nn.Module):
    def __init__(self, dim, channels=4):  # 修改：默认4通道（3+1）
        super(DiffusionNet, self).__init__()
        self.net = Unet(
            dim=dim,
            channels=channels,
            dim_mults=(1, 2, 4, 8),
            with_cat_emb=True  # 启用类别嵌入
        )
