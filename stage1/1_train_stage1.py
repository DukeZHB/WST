        ...
        for iter, (filename, data, label) in enumerate(train_data_loader):
            ...
            if (optimizer.global_step)%100 == 0 and (optimizer.global_step)!=0:
                ...
                print('Epoch:%2d' % (ep),
                      ...
                      flush=True)
    torch.save(model.state_dict(), os.path.join(args.save_folder, 'stage1_checkpoint_'+ args.version + '_trained_on_'+args.dataset+'.pth'))

def test_phase(args):
    model = getattr(importlib.import_module(args.network), 'Net_CAM')(n_class=args.n_class)
    model = model.cuda()
    args.weights = os.path.join(args.save_folder, 'stage1_checkpoint_'+ args.version + '_trained_on_'+args.dataset+'.pth')
    weights_dict = torch.load(args.weights)
    model.load_state_dict(weights_dict, strict=False)
    model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--PMroot", default='./datasets/BCSS-WSSS/train_CAM/', type=str)
    parser.add_argument("--dataset", default='bcss', type=str)
    parser.add_argument("--save_folder", default='checkpoints', type=str)
    # parser.add_argument("--trainroot", default='./datasets/LUAD-HistoSeg/train/img/', type=str)
    # parser.add_argument("--testroot", default='./datasets/LUAD-HistoSeg/test/', type=str)
    # parser.add_argument("--PMroot", default='./datasets/LUAD-HistoSeg/train_CAM/', type=str)
    # parser.add_argument("--dataset", default='luad', type=str)
    # parser.add_argument("--trainroot", default='./datasets/WSSS4LUAD/train/img/', type=str)
    # parser.add_argument("--testroot", default='./datasets/WSSS4LUAD/test/', type=str)
    # parser.add_argument("--PMroot", default='./datasets/WSSS4LUAD/train_PM/', type=str)
    # parser.add_argument("--dataset", default='wsss', type=str)
    parser.add_argument("--version", default="1.10cam", type=str)
    args = parser.parse_args()
    # 新增代码：创建logs文件夹（如果不存在）
    log_dir = 'logs'
    os.makedirs(log_dir, exist_ok=True)  # 确保文件夹存在，exist_ok=True避免重复创建报错

    # 定义日志文件路径
    log_file = os.path.join(log_dir, f'stage1_log_{args.version}_trained_on_{args.dataset}.txt')


    # 重定向标准输出到日志文件（同时保留控制台输出）
    class Logger:
